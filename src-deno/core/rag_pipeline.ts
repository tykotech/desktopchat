...
export async function executeRagPipeline(sessionId: string, userMessage: string) {
  ...
  try {
    ...
    if (knowledgeBases.length > 0) {
      try {
        ...
        const embeddingClient = await LLMFactory.getClientForModel(embeddingModel);
        
        // Generate embedding for the user's query
        const queryEmbeddings = await embeddingClient.generateEmbeddings([userMessage], embeddingModel);
        ...
        context = searchResults.map((result: any) => result.payload.content).join("\n\n");
    ...
    let webResults: { title: string; url: string; snippet: string }[] | undefined;
    
    try {
      // Check if we should perform web search
      // This could be based on settings or the quality of the retrieved context
      const shouldWebSearch = context === "No relevant context found." || searchResults.length < 3;
    ...
    for await (const chunk of responseStream) {
      emitEventToFrontend(`chat-stream-chunk-${sessionId}`, { content: chunk.content });
    ...
    console.log(`Response generation completed in ${generationTime}ms with ${chunkCount} chunks`);
